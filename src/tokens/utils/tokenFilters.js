...
function matchThemableToken(
	token,
	modifierPathSegment,
  tokenPaths = ['color', 'container', 'form-item', 'tooltip', 'panel']
) {
	const tokenPath = token.path.join('.').toLowerCase();
	return (
		tokenPaths.some((p) => tokenPath.startsWith(p)) && token.path.includes(modifierPathSegment)
	);
}
...
export { filteredTokens, matchThemableToken, matchDarkThemeToken, matchLightThemeToken };
